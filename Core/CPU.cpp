...
void CPU::clocked() {
	...
	if(this->mem.ppu.actualScanlineNum == -1 && this->mem.ppu.actualCycleNum >= 338 && this->mem.ppu.actualCycleNum <= 340 &&
		this->mem.ppu.isRenderingBG())
		this->mem.ppu.actualCycleNum += this->mem.ppu.oddFrame;
	if(this->mem.ppu.actualCycleNum > 340) {
		this->mem.ppu.actualCycleNum -= 341;
		this->mem.ppu.actualScanlineNum = ((this->mem.ppu.actualScanlineNum + 1 + 1) % (261 + 1)) - 1;
	}
	// Rendering screen or NMI
	if(this->mem.ppu.actualCycleNum > 0 && this->mem.ppu.actualCycleNum <= 3 &&
		this->mem.ppu.actualScanlineNum == 240 || this->mem.ppu.actualScanlineNum == 241)
